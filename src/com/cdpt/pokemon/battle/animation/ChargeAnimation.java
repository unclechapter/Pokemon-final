...
public class ChargeAnimation extends BattleAnimation {
	...
	public void initialize(AssetManager assetManager, TweenManager tweenManager) {
		...
		Tween.to(this, BattleAnimationAccessor.PRIMARY_OFFSET_X, getDuration() / 2).target(1f).start(tweenManager);
		Tween.to(this, BattleAnimationAccessor.PRIMARY_OFFSET_X, getDuration() / 2).target(0f).delay(getDuration() / 2).start(tweenManager);
		Tween.to(hitmark, BattleSpriteAccessor.ALPHA, 0f).target(1f).delay(getDuration() / 10 * 3).start(tweenManager);
		Tween.to(hitmark, BattleSpriteAccessor.ALPHA, 0f).target(0f).delay(getDuration() / 10 * 7).start(tweenManager);
	}
}
